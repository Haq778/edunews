<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      >
        <div v-if="isFilterPanelOpen" class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 mb-8">
          <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <!-- Search -->
            <div class="lg:col-span-2">
              <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">🔍 Cari Berita</label>
              <input 
                v-model="searchQuery"
                type="text"
                placeholder="Ketik kata kunci..."
                class="w-full p-4 rounded-xl border border-gray-200 dark:border-gray-600 focus:outline-none focus:ring-3 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              />
            </div>
            ...
            <div>
              ...
              <select v-model="selectedCategory" class="w-full p-4 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            ...
            <!-- Sort -->
            <div>
              <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">🔄 Urutkan</label>
              <select v-model="sortBy" class="w-full p-4 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
      ...
      <div v-if="error" class="text-center py-20">
        <p class="text-red-500 dark:text-red-400">Gagal memuat berita: {{ error.message }}</p>
      </div>

      <!-- News List/Grid -->
      <div v-if="!loading && !error" id="news-section">
        <div :class="viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6' : 'grid grid-cols-1 gap-6'">
          <NewsCard 
            v-for="(b, index) in sortedBerita" 
            :key="b.id" 
            :berita="b"
            :view-mode="viewMode"
            :index="index"
...
const sortedBerita = computed(() => {
  ...
  switch(sortBy.value) {
    case 'newest': return articles.sort((a,b) => new Date(b.date) - new Date(a.date))
    case 'oldest': return articles.sort((a,b) => new Date(a.date) - new Date(b.date))
    case 'title': return articles.sort((a,b) => a.title.localeCompare(b.title))
    case 'views': return articles.sort((a,b) => b.views - a.views)
...
<style scoped>
/* Tambahkan animasi atau style khusus jika mau */
</style>
